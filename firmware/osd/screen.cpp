...
panel_pos_t records [OSD_DEFAULT_SCREEN_PANELS];
uint8_t count = 0;

uint8_t *eeprom_offset (uint8_t num, uint8_t panel)
{
	return _eeprom_byte (OSD_SCREENS_EEPROM_OFFSET + num * sizeof (panel_pos_t) * OSD_DEFAULT_SCREEN_PANELS  + panel * sizeof (panel_pos_t));
}

void load (uint8_t num)
{
	if (num >= OSD_DEFAULT_SCREENS) num = 0;

	uint8_t *offset = eeprom_offset (num);

	uint8_t i = 0;
	for (; i < OSD_DEFAULT_SCREEN_PANELS; i ++)
	{
...
{
	uint8_t *offset = eeprom_offset (num);
	for (uint8_t i = 0; i < OSD_DEFAULT_SCREEN_PANELS; i ++, offset += sizeof (panel_pos_t))
	{
		if (i < len)
		{
